...
const Header: React.FC<HeaderProps> = ({ onLoginClick, onNavigate, currentPage }) => {
  return (
    <header className="py-4 px-6 md:px-12 flex justify-between items-center border-b border-slate-800/50 sticky top-0 bg-gray-900/80 backdrop-blur-sm z-50">
      <div className="flex items-center cursor-pointer" onClick={() => onNavigate('home')}>
        ...
        <div className="flex flex-col">
            <span className="text-xl font-bold leading-none">Social Media Minion</span>
            <span className="text-[10px] text-gray-500 font-mono mt-1">imagination 04.11.1963</span>
        </div>
      </div>
      ...
      <button onClick={onLoginClick} className="px-5 py-2 font-semibold bg-slate-800 hover:bg-slate-700 rounded-lg transition-colors cursor-pointer">
        Log In
      </button>
    </header>
  );
