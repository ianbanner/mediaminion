...
const Sidebar: React.FC<SidebarProps> = ({ 
  ...
  return (
    <div className="relative w-72 bg-slate-900/50 border-r border-slate-800 p-4 flex flex-col flex-shrink-0">
      <div className="mb-8">
         <button 
            className="w-full text-xl font-bold text-white flex items-start text-left cursor-pointer group"
      ...
      <nav className="flex-1 space-y-2">
            <AccordionItem title="Work on Posts" icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>} isOpen={openMenu === 'posts'} onToggle={() => handleMenuToggle('posts')}>
                ...
                <NavItem label="Quick Post" viewName="quick-post" currentView={view} onClick={() => handleNavClick('quick-post')}><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg></NavItem>
            ...
            <AccordionItem title="Work on Articles" icon={<svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.246 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>} isOpen={openMenu === 'articles'} onToggle={() => handleMenuToggle('articles')}>
                ...
                <NavItem label="Quick Article" viewName="quick-article" currentView={view} onClick={() => handleNavClick('quick-article')}><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg></NavItem>
      ...
      {showMobileMenu && (
        <div className="absolute inset-0 bg-slate-900/95 z-10 p-4 animate-fade-in-fast">
            <div className="flex justify-between items-center mb-8">
                <h3 className="text-lg font-bold">Quick Actions</h3>
                <button onClick={onToggleMobileMenu} className="p-2 text-gray-400 hover:text-white">&times;</button>
            </div>
            <div className="space-y-4">
                <button
                    onClick={() => { setView('quick-post'); setShowMobileMenu(false); }}
                    className="w-full text-left p-4 rounded-lg bg-slate-800 hover:bg-slate-700 text-lg font-semibold flex items-center gap-4"
                >
                    <svg className="w-8 h-8 text-teal-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path></svg>
                    Quick Post
                </button>
                 <button
                    ...
                    className="w-full text-left p-4 rounded-lg bg-slate-800 hover:bg-slate-700 text-lg font-semibold flex items-center gap-4"
                >
                    <svg className="w-8 h-8 text-indigo-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                    Quick Article
                </button>
            </div>
        </div>
