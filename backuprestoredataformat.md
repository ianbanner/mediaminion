...
| `ayrshareQueue`                 | `array`       | An array of posts waiting to be published. See `QueuedPost` structure below.                          |
...
| `generatedArticleIdeas`         | `array`\|`null` | The list of article ideas from Step 1 of the headline generator.                                        |
| `selectedArticleIdea`           | `string`\|`null` | The article idea selected by the user to generate headlines for.                                        |
...
### `QueuedPost`
Each object in the `ayrshareQueue` array.
```json
{
  "title": "The List of Lessons",
